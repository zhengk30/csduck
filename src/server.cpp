...
// #define DBFILE_PATH "/users/zhengk30/csduck/inputs/sf10.db"
#define DBFILE_PATH "/proj/fardatalab-PG0/dbfiles/sf100.db"
#define IMDB_PATH "/proj/fardatalab-PG0/exported-data/sf1"
...
void process_request(int client_sock, const char* src, bool prof_brkdown_enabled) {
    char query[BUF_SIZE];
    ...
    char* import = "IMPORT DATABASE '";
    memcpy(import_query, import, strlen(import));
    memcpy(import_query + strlen(import), src, strlen(src));
    memcpy(import_query + strlen(import) + strlen(src), "';", 2);
    printf("import query: %s\n", import_query);
    ...
    assert(duckdb_open(NULL, &db) != DuckDBError);
...
void process_request(int client_sock, bool prof_brkdown_enabled) {
    ...
    // retrieve result from row 0, column 0 of the returned table
    uint64_t count = 0;
    if (duckdb_row_count(&result) > 0) {
        count = duckdb_value_uint64(&result, 0, 0);
    }
